...
def run_with_synthetic_data():
    ...
    random.seed(73)
    R_init = dot(R_true, SO3.exp(random.randn(3)*.2))
    ...
    print '\nTrue [R t]:'
    print np.hstack((R_true, t_true[:,newaxis]))
    print '\nInitial [R t]:'
    ...
    #fr = lambda v: cost(K, dot(R_true, SO3.exp(v[:3])), t_true+v[3:], unpr(xs0), unpr(xs1))
    #fr_reduced = lambda v: cost(K, dot(R_true, SO3.exp(v[:3])), t_true+[v[3],v[4],0.], unpr(xs0), unpr(xs1))
    #H_true = finite_differences.numeric_hessian(fr, zeros(6)).astype(float)
    #H_true_reduced = finite_differences.numeric_hessian(fr_reduced, zeros(5)).astype(float)

    #print 'Hessian at true'
    #print H_true
    #evals,evecs = eig(squeeze(H_true))
    #order = argsort(evals)
    #print 'Eigenvalues of Hessian at true:'
    #print evals[order].round(3)
    #print 'Eigenvectors of Hessian at true:'
    #print evecs[order].round(2)
